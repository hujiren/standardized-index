...
@Component
@Service
public class ModuleDistrictBusinessHandler implements ModuleHandler {
  ...
  public boolean support(String type) {
    return ModuleEnum.MODULE_DISTRICT_BUSINESS.name().equals(type);
  }
   ...
   * 计算商圈环比等.
   */
  public void calBusinessRatio(StatementParam param, ModuleDistrictHotpotBaseValue value) {
    List<HotpotInfoDto> thisList = dao
        .getBusinessInfo(param.getCurrentStartDate(), param.getCurrentEndDate(),
            param.getDistrictId());
    List<HotpotInfoDto> lastList = dao
        .getBusinessInfo(param.getCycleStartDate(), param.getCycleEndDate(),
            param.getDistrictId());
