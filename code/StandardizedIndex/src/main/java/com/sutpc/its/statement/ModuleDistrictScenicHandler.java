...
@Component
@Service
public class ModuleDistrictScenicHandler implements ModuleHandler {
  ...
  public boolean support(String type) {
    return ModuleEnum.MODULE_DISTRICT_SCENIC.name().equals(type);
  }
   ...
   * 计算景区环比等.
   */
  public void calScenicRatio(StatementParam param, ModuleDistrictHotpotBaseValue value) {
    List<HotpotInfoDto> thisList = dao
        .getScenicInfo(param.getCurrentStartDate(), param.getCurrentEndDate(),
            param.getDistrictId());
    List<HotpotInfoDto> lastList = dao
        .getScenicInfo(param.getCycleStartDate(), param.getCycleEndDate(),
            param.getDistrictId());
