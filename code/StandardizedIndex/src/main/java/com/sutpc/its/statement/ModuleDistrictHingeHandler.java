 ...
 *
 * @Author: zuotw
 * @Date: created on 15:36 2020/6/1.
 * @Description
 * @Modified By:
 */
@Component
@Service
public class ModuleDistrictHingeHandler implements ModuleHandler {
  ...
  public boolean support(String type) {
    return ModuleEnum.MODULE_DISTRICT_HINGE.name().equals(type);
  }
   ...
   * 计算枢纽环比等.
   */
  public void calHingeRatio(StatementParam param, ModuleDistrictHotpotBaseValue value) {
    List<HotpotInfoDto> thisList = dao
        .getSchoolInfo(param.getCurrentStartDate(), param.getCurrentEndDate(),
            param.getDistrictId());
    List<HotpotInfoDto> lastList = dao
        .getSchoolInfo(param.getCycleStartDate(), param.getCycleEndDate(),
            param.getDistrictId());
